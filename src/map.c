...
static inline guint get_direction_type (char *text)
{
    /* Can easily be optimised ... but why bother ? */
         ...
         if (!strcasecmp(text, "n"     )) return NORTH;
    else if (!strcasecmp(text, "ne"    )) return NORTHEAST;
    else if (!strcasecmp(text, "e"     )) return EAST;
    else if (!strcasecmp(text, "se"    )) return SOUTHEAST;
    else if (!strcasecmp(text, "s"     )) return SOUTH;
    else if (!strcasecmp(text, "sw"    )) return SOUTHWEST;
    else if (!strcasecmp(text, "w"     )) return WEST;
    else if (!strcasecmp(text, "nw"    )) return NORTHWEST;
    else if (!strcasecmp(text, "up"    )) return UP;
    else if (!strcasecmp(text, "down"  )) return DOWN;
    else if (!strcasecmp(text, "remove")) return REMOVE;
    else if (!strcasecmp(text, "load"  )) return LOAD;
    else if (!strcasecmp(text, "save"  )) return SAVE;
